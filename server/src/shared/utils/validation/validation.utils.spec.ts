...
describe('ValidationUtils', () => {
	...
	test('valid minLength exclusive', () => {
		const value = 'abc';
		const min = 2;
		expect(ValidationUtils.minLength(value, min, true)).toBe(true);
	});
	test('invalid minLength exclusive', () => {
		const value = 'ab';
		const min = 2;
		expect(ValidationUtils.minLength(value, min, true)).toBe(false);
	});

	test('valid maxLength exclusive', () => {
		const value = 'abc123';
		const max = 7;
		expect(ValidationUtils.maxLength(value, max, true)).toBe(true);
	});
	test('invalid maxLength exclusive', () => {
		const value = 'abc1234';
		const max = 7;
		expect(ValidationUtils.maxLength(value, max, true)).toBe(false);
	});
